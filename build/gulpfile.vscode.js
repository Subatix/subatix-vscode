...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, opts) {
	...
	const task = () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(json({ commit, date: readISODate('out-build'), checksums, version, serverDownloadUrlTemplate: 'https://github.com/subatixinc/subatix/releases/download/1.106.38207/subatix-reh-${os}-${arch}-1.106.38207.tar.gz' }))
			.pipe(es.through(function (file) {
