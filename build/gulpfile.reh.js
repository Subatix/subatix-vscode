...
function packageTask(type, platform, arch, sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(json({ commit, date: readISODate('out-build'), version, serverDownloadUrlTemplate: 'https://github.com/subatixinc/subatix/releases/download/1.106.38207/subatix-reh-${os}-${arch}-1.106.38207.tar.gz' }))
			.pipe(es.through(function (file) {
